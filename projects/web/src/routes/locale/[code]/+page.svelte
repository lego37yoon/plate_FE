...
<svetle:head>
  <title>{m["title.l10n"]({ lang_code: data.projects[0].code })} : Plate</title>
</svetle:head>

<section id="local_team_desc">
  <h1 class="text-3xl text-primary">{m["title.l10n"]({ lang_code: data.projects[0].code })}</h1>
  <p class="text-primary flex gap-1 items-center mb-4">
    <a class="font-light" href={localizeHref("/locale")}>{m["menu.lang"]()}</a>
    <ChevronRight size={16} />
    <span class="font-semibold">{data.projects[0].code}</span>
  </p>
    ...
    <tbody>
      ...
      <tr>
        <td class="min-w-1/3 w-1/2 shrink text-start p-2">
          <a href={localizeHref(`/locale/${project.code}/${project.project_id}`)}>
            {project.projects.name}
          </a>
        </td>
        <td class="text-end">{project.projects.version}</td>
        <td class="text-end">{project.projects.files[0].count}</td>
        <td></td>
      </tr>
      {/each}
    </tbody>
